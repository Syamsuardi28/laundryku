...
@section('content')
<x-ui.page-header title="Edit Pelanggan" subtitle="Perbarui data pelanggan">
    <x-slot:actions>
...
<div class="mx-auto max-w-xl">
    <div class="card p-6">
        <div class="mb-6 flex items-center gap-3">
            <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-primary-50 to-blue-100 text-2xl font-bold text-primary dark:from-primary/20 dark:to-blue-500/20 dark:text-primary-100">
                {{ strtoupper(substr($customer->nama, 0, 1)) }}
            </div>
            <div>
                <h2 class="font-semibold text-slate-900 dark:text-white">{{ $customer->nama }}</h2>
                <p class="text-sm text-slate-500 dark:text-slate-400">ID Pelanggan #{{ $customer->id }}</p>
            </div>
        </div>

        <form method="POST" action="{{ route('customers.update', $customer) }}" class="space-y-5" data-loading id="form-edit-pelanggan-{{ $customer->id }}">
            @csrf @method('PUT')
            ...
            <div class="space-y-1">
                ...
                <div class="relative">
                    ...
                    <input type="text" name="telepon" id="telepon" value="{{ old('telepon', $customer->telepon) }}"
            ...
            <div class="space-y-1">
                ...
                <textarea name="alamat" id="alamat" rows="3"
                          ...
                          class="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 placeholder-slate-400 transition-all focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 dark:placeholder-slate-500 resize-y @error('alamat') border-danger focus:border-danger focus:ring-danger/20 @enderror">{{ old('alamat', $customer->alamat) }}</textarea>
                @error('alamat')
                    <p class="flex items-center gap-1.5 text-xs font-medium text-danger">
                        <svg class="h-3.5 w-3.5 shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>
                        {{ $message }}
            ...
            <div class="flex gap-3 pt-2 border-t border-slate-100 dark:border-slate-700">
                <button type="submit" class="btn-primary flex-1" id="btn-update-pelanggan">
